...
page = st.sidebar.radio("Select a Page", ["Home", "Specialization Recommendation", "University Recommendation"])
...
# Streamlit UI
# ---------------------------
st.title("🎓 Specialization & University Recommender System")
# Navigation Sidebar
...
if page == "Home":
    ...
    st.write("""
    This system helps you find the most suitable *specializations* and *universities* for your academic and professional background.
    
    - Go to *Specialization Recommendation* to find potential academic specialization courses.
...
# Streamlit UI for Course Recommendation
elif page == "Specialization Recommendation":
    models = load_specialization_models()
    st.header("📌 Specialization Recommendation")
    user_input = {
    ...
    if st.button("🔍 Recommend Specialization"):
        ...
        st.success("🎯 Recommended Specialization:")
        for r in recs:
 ...
 #       "ugCollege": st.selectbox("Undergraduate College", sorted(df['ugCollege'].dropna().unique())),
        "ugCollege": st.selectbox("Undergraduate College", sorted([college for college in df['ugCollege'].dropna().unique() if college != 0])),
